...
export const getTrendingTagCollection = (products, type, tag) => {
    const items = products.filter(product => {
        return product.category === type && product.tags && product.tags.includes(tag);
    })
    return items.slice(0,8)
}

export const getPremiumTagCollection = (products, type, tag) => {
    const items = products.filter(product => {
        return product.category === type && product.tags && product.tags.includes(tag);
    })
    return items.slice(0,8)
...
export const getMultipleTagCollection = (products, type, tags) => {
    // Filter products based on category and whether any of the tags match
    const items = products.filter(product => {
        return product.category === type && product.tags && tags.some(tag => product.tags.includes(tag));
    });

    // Return a limited number of items
    return items.slice(0, 8);
};
export const getRegularTagCollection = (products, type, tag) => {
...
export const getSaveAndExtraTagCollection = (products, type, tag) => {
    const items = products.filter(product => {
        return product.category === type && product.tags && product.tags.includes(tag);
    })
    return items.slice(0,8)
...
//Products Tab
export const getProductTagCollections = (allproducts, type, tag) => {
...
export const getSearchCollections = (allproducts, str) => {
    const products =allproducts['products'];
    return products;
}
//Category Tab

export const getCategoryTagCollections = (allproducts, type, tag) => {
    const products =allproducts['products'];
    const items = products.filter(product => {
        return product.category === type && product.tags && product.tags.includes(tag);
    })
...
export const getCategoryTagTypeCollections = (allproducts, category, tag, type) => {
    ...
    if (type === "all") {
        const items = products.filter(product => {
            return product.category === category && product.tags && product.tags.includes(tag);
        })
        return items.slice(0)
    } else {
        const items = products.filter(product => {
            return product.category === category && product.tags && product.tags.includes(tag) && product.type === type;
        })
        return items.slice(0)
    }
}
...
export const getBestSellerProducts = products => {
    const items = products.filter(product => {
        return product.bestSeller === true;  // Now treats as boolean
    })
    //sort by visibleOrder in ascending order
    return items.sort((a, b) => (a.visibleOrder || 0) - (b.visibleOrder || 0)).slice(0,8)
}

// Get Best Seller
export const getBestSeller = products => {
    const items = products.filter(product => {
        return product.bestSeller > 0;
    })
    //sort in ascending order
    return items.sort((a, b) => a.bestSeller - b.bestSeller).slice(0,8)
    // return items.slice(0,8)
}
