...
import {removeFromCompare, addToCartHome} from '../../actions'
...
class Compare extends Component {
    ...
    render (){
        ...
        const {Items, symbol, addToCartHome, removeFromCompare} = this.props;
                                        ...
                                        <div key={index}>
                                            <div className="compare-part">
                                                ...
                                                <div className="img-secton">
                                                   <Link href={`/view/product/${item.id}/${item.name}`}>
                                                    <img src={item.variants?
                                                                ...
                                                                :getImage(item.images)} className="img-fluid" alt="" />
                                                    <h5>{item.name}</h5></Link>
                                                    <h5>{symbol}{(item.price)}
                                                        <del><span className="money">{symbol}{item.mrpPrice}</span></del></h5>
                                                </div>
                                                ...
                                                <div className="btn-part">
                                                    <button className="btn btn-solid" onClick={() => addToCartHome(item, 1,item.variantCode)}>add to cart</button>
                                                </div>
                                            </div>
                                        </div>
                                    )}
...
export default connect(
    mapStateToProps,
    {removeFromCompare, addToCartHome}
)(Compare)