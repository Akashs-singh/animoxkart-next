...
import SimilarProducts from "../common/similar-products";
...
class LeftSideBar extends Component {
    ...
    render() {
        ...
        var descriptionContent= "Animoxkart | Pet Store | " +item?.displayName + " - " + item?.shortDetails;
        var keywordsContent= "Animoxkart | Pet Store - " + item?.displayName;
        return (
            <div>
                {/*SEO Support*/}
                {/*SEO Support End */}

                {this.state.isMounted && <Breadcrumb parent={'Product'} title={item?.displayName} />}
                {/*Section Start*/}
                {this.state.isMounted && (item) ?
                    <section className="section-b-space">
                        <div className="collection-wrapper">
                            ...
                            <Slider {...products} asNavFor={this.state.nav2} ref={slider => (this.slider1 = slider)} className="product-slick">
                                                        {item?.variants?
                                                            item?.variants.filter(variant => variant.productCode === this.state.variantCode).map((vari, i) =>
                                                                vari.pictures.map((pic, index) =>
                                                                    <div key={index}>
                                                                        <ImageZoom image={getImage(pic.image)} />
                                                                    </div>
                                                                )
                                                            ) :
                                ...
                                {/* <div className="row"> */}
                                    <SimilarProducts type={'wearable'} title="Similar Products" subtitle="Special Offer" />
                                {/* </div> */}
                            </div>
                        </div>
                    </section> : ''}
